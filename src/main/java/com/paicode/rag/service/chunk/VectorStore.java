 ...
 * @Description SQLite 向量存储 + 代码关系图谱持久化
 * 向量以 JSON 的形式存储在 SQLite 中, 检索时在内存计算余弦相似度
 */
public class VectorStore implements AutoCloseable {
    ...
    public VectorStore(String projectPath) throws SQLException {
        ...
        // 创建连接
        String dbDir = System.getProperty("paicode.rag.dir", System.getProperty("user.home") + "/.paicode/rag");
    ...
    public List<SearchResult> search(float[] queryEmbedding, int topK) throws SQLException {
        String sql = """
                SELECT file_path, chunk_type, name, content, embedding_json FROM code_chunks 
                WHERE project_path = ?
                    """;
